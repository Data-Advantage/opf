...
var require_error = __commonJS({
  "node_modules/.pnpm/commander@12.1.0/node_modules/commander/lib/error.js"(exports$1) {
...
var require_argument = __commonJS({
  "node_modules/.pnpm/commander@12.1.0/node_modules/commander/lib/argument.js"(exports$1) {
...
var require_help = __commonJS({
  "node_modules/.pnpm/commander@12.1.0/node_modules/commander/lib/help.js"(exports$1) {
...
var require_option = __commonJS({
  "node_modules/.pnpm/commander@12.1.0/node_modules/commander/lib/option.js"(exports$1) {
    var { InvalidArgumentError: InvalidArgumentError2 } = require_error();
    var Option2 = class {
      /**
...
var require_suggestSimilar = __commonJS({
  "node_modules/.pnpm/commander@12.1.0/node_modules/commander/lib/suggestSimilar.js"(exports$1) {
    var maxDistance = 3;
...
var require_command = __commonJS({
  "node_modules/.pnpm/commander@12.1.0/node_modules/commander/lib/command.js"(exports$1) {
...
var require_commander = __commonJS({
  "node_modules/.pnpm/commander@12.1.0/node_modules/commander/index.js"(exports$1) {
...
// node_modules/.pnpm/@pptx+sdk@file+..+typescript/node_modules/@pptx/sdk/dist/index.js
var PptxError = class extends Error {
  constructor(message) {
    super(message);
    this.name = "PptxError";
  }
};
var PptxNetworkError = class extends PptxError {
  cause;
...
var PptxApiError = class extends PptxError {
  status;
  requestId;
  body;
...
var PptxValidationError = class extends PptxApiError {
  validationErrors;
  constructor(message, status, body, requestId, validationErrors) {
    super(message, status, body, requestId);
    this.name = "PptxValidationError";
    this.validationErrors = validationErrors;
  }
};
var PptxRateLimitError = class extends PptxApiError {
  retryAfterSeconds;
...
var DEFAULT_BASE_URL = "https://api.pptx.dev";
...
function errorMessage(status, body) {
  if (body && typeof body === "object" && typeof body.error === "string") {
    return `pptx.dev API ${status}: ${body.error}`;
  }
...
var PptxClient = class {
  ...
  async request(options) {
    ...
    if (!response.ok) {
      ...
      if (response.status === 422 && body && typeof body === "object" && Array.isArray(
      ...
      if (response.status === 429) {
        ...
        throw new PptxRateLimitError(
          ...
          requestId,
          Number.isFinite(retryAfterSeconds) ? retryAfterSeconds : void 0
        );
      }
...
var OpfNamespace = class {
   ...
   */
  async validate(document, signal) {
  ...
  async generate(document, options) {
    return this.client.request({
      method: "POST",
      path: "/v1/generate",
      query: options?.format ? { format: options.format } : void 0,
      body: document,
      signal: options?.signal
    });
  }
};
var ParseNamespace = class {
  constructor(client) {
    this.client = client;
  }
  client;
  ...
  async slide(parseId, index, signal) {
    return this.client.request({
      method: "GET",
...
var RenderNamespace = class {
  constructor(client) {
    this.client = client;
  }
  client;
  /** Render a .pptx to web slides (interactive HTML viewer). */
  async web(file, options) {
    const form = new FormData();
    const { blob, filename } = toFilePart(file);
    form.append("file", blob, filename);
    const query = { format: "web" };
  ...
  async export(file, format, options) {
    const form = new FormData();
    const { blob, filename } = toFilePart(file);
    form.append("file", blob, filename);
    const query = { format };
...
var ConvertNamespace = class {
  constructor(client) {
    this.client = client;
  }
  client;
  /** Convert a .pptx to OPF JSON. */
  async pptxToOpf(file, signal) {
    const form = new FormData();
    const { blob, filename } = toFilePart(file);
    form.append("file", blob, filename);
    return this.client.request({
...
var CONFIG_DIR = path.join(os.homedir(), ".pptx");
...
function resolveApiKey2(explicit) {
...
async function readLinePlain(question) {
...
function loginCommand() {
  return new Command("login").description("Save a pptx.dev API key to ~/.pptx/config.json (0600)").option("--api-key <key>", "API key (skip interactive prompt)").option("--base-url <url>", "Override the API base URL").action(async (opts) => {
    let key = opts.apiKey;
    if (!key) {
...
function logoutCommand() {
  return new Command("logout").description("Remove saved pptx.dev credentials").action(() => {
...
function validateCommand() {
  return new Command("validate").description("Validate an OPF JSON document against the canonical schema").argument("<file>", "Path to an OPF JSON file").option("--json", "Emit the raw API response as JSON on stdout").action(async (file, opts, cmd) => {
...
function parseCommand() {
  return new Command("parse").description("Parse a .pptx file into OPF JSON via the pptx.dev API").argument("<file>", "Path to a .pptx file").option("-o, --output <file>", "Write OPF JSON to this file (default: stdout)").action(async (file, opts, cmd) => {
    const globals = cmd.parent?.opts() ?? {};
    const client = buildClient(globals);
    const path$1 = path.resolve(file);
    let data;
    try {
      data = fs.readFileSync(path$1);
    } catch (err) {
      throw new CliError(
        `Could not read ${file}: ${err instanceof Error ? err.message : String(err)}`,
        2
    ...
    if (opts.output) {
      fs.writeFileSync(opts.output, `${JSON.stringify(opf, null, 2)}
`);
...
var SUPPORTED_FORMATS = ["pptx", "pdf", "png", "svg"];
function renderCommand() {
  return new Command("render").description("Render an OPF JSON document to a target format").argument("<file>", "Path to an OPF JSON file").requiredOption(
    "-f, --format <format>",
    ...
    printJson(response);
    info(`render accepted \u2014 format=${opts.format} slides=${response.slideCount}`);
  });
}
function fmtCommand() {
  return new Command("fmt").description("Format/normalize an OPF JSON document (local, no API call)").argument("<file>", "Path to an OPF JSON file").option("-w, --write", "Write the formatted output back to the file").option("--check", "Exit 1 if the file is not already formatted").action((file, opts) => {
    const path$1 = path.resolve(file);
    let raw;
    try {
      raw = fs.readFileSync(path$1, "utf8");
...
`;
    ...
    if (opts.write) {
      fs.writeFileSync(path$1, formatted);
      info(`formatted ${file}`);
      return;
    }
...
function addCommand() {
  return new Command("add").description("Scaffold OPF component snippets (slide types, themes)").argument("<component>", "Component to scaffold").option("-o, --output <file>", "Write the snippet to this file instead of stdout").addHelpText(
    "after",
    `
Available components:
${Object.keys(SNIPPETS).sort().map((k) => `  - ${k}`).join("\n")}
`
    ...
    if (opts.output) {
      const path$1 = path.resolve(opts.output);
      fs.writeFileSync(path$1, `${JSON.stringify(snippet, null, 2)}
`);
...
var SUPPORTED_FORMATS2 = ["pptx", "pdf", "png", "svg"];
function generateCommand() {
  return new Command("generate").description("Generate a presentation from an OPF JSON file or a prompt").argument("<input>", "Path to an OPF JSON file, or a natural-language prompt").option(
    "-f, --format <format>",
    `Output format: ${SUPPORTED_FORMATS2.join(" | ")}`,
    "pptx"
  ).option(
    "-o, --output <file>",
    "Write the generated OPF JSON to this file (for prompt mode)"
  ).action(
    async (input, opts, cmd) => {
      const globals = cmd.parent?.opts() ?? {};
      if (!SUPPORTED_FORMATS2.includes(opts.format)) {
        throw new CliError(
          `Unsupported format "${opts.format}". Supported: ${SUPPORTED_FORMATS2.join(", ")}.`,
          2
        );
      }
      const client = buildClient(globals);
      if (looksLikeFile(input)) {
        ...
        if (opts.output) {
          fs.writeFileSync(path.resolve(opts.output), `${JSON.stringify(doc, null, 2)}
`);
      ...
      info(
        "scaffold one with `pptx add deck -o deck.opf.json`, edit it, then run `pptx generate deck.opf.json`."
      );
      throw new CliError("prompt generation not supported yet", 3);
    }
  );
}
function looksLikeFile(input) {
...
async function main(argv) {
  ...
  program2.name("pptx").description("Official CLI for pptx.dev \u2014 generate, parse, validate, and render OPF presentations.").version(CLI_VERSION).option("--api-key <key>", "Override the API key for this call").option("--base-url <url>", "Override the API base URL");
  ...
  program2.addCommand(parseCommand());
  program2.addCommand(renderCommand());
  program2.addCommand(fmtCommand());
  program2.addCommand(addCommand());
  program2.showHelpAfterError();
  program2.showSuggestionAfterError();
